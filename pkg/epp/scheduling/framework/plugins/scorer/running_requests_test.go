...
func TestRunningRequestsScorer(t *testing.T) {
	tests := []struct {
		name              string
		pods              []types.Pod
	...
	}{
		{
			...
			expectedScoresPod: map[int]float64{
				...
				1: 0.5, // Medium running requests (5) gets medium score
				2: 1.0, // Least running requests (0) gets highest score
			},
		},
		{
	...
	scorer := NewRunningRequestsScorer()
