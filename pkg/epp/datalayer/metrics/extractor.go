...
func NewExtractor(queueSpec, runningSpec, kvusageSpec, loraSpec, cacheInfoSpec, runningRequestsSpec, gpuUtilizationSpec string) (*Extractor, error) {
	mapping, err := NewMapping(queueSpec, runningSpec, kvusageSpec, loraSpec, cacheInfoSpec, runningRequestsSpec, gpuUtilizationSpec)
	if err != nil {
...
func (ext *Extractor) Extract(ctx context.Context, data any, ep datalayer.Endpoint) error {
	...
	if spec := ext.mapping.RunningRequests; spec != nil { // extract running requests
		if metric, err := spec.getLatestMetric(families); err != nil {
			errs = append(errs, err)
		} else {
			clone.RunningQueueSize = int(extractValue(metric))
